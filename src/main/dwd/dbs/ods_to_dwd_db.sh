...
operate_time,
expire_time
from ${APP}.ods_coupon_info where dt='$do_date';
...
end_time,
create_time
from ${APP}.ods_activity_info where dt='$do_date';
...
oi.province_id
from
( select * from ${APP}.ods_payment_info where  dt='$do_date' )pi
join
( select
id,
province_id
from ${APP}.ods_order_info where dt = '$do_date' )oi
...
refund_reason_type,
create_time
from ${APP}.ods_order_refund_info where dt='$do_date';
...
source_type,
source_id
from ${APP}.ods_cart_info where dt='$do_date';
...
create_time,
cancel_time
from ${APP}.ods_favor_info where dt='$do_date';
...
using_time,
used_time
from ${APP}.dwd_fact_coupon_use where dt in
(
select
date_format(get_time,'yyyy-MM-dd') from ${APP}.ods_coupon_use
where dt='$do_date'
) )old
full outer join
(
...
if(new.feight_fee is null,old.feight_fee,new.feight_fee), if(new.final_total_amount is
null,old.final_total_amount,new.final_total_amount), date_format(if(new.tms['1001'] is
...
feight_fee,
final_total_amount
from ${APP}.dwd_fact_order_info where dt
in
(
...
select
info.id,
info.order_status, info.user_id, info.out_trade_no, info.province_id, act.activity_id,
log.tms, info.original_total_amount, info.benefit_reduce_amount, info.feight_fee, info.final_total_amount
from (select order_id,
str_to_map(concat_ws(',',collect_set(concat(order_status,'=',operate_time))),',',' =') tms
from ${APP}.ods_order_status_log where dt='$do_date'
group by order_id
)log join
(
select * from ${APP}.ods_order_info where dt='$do_date'
...
(
select * from ${APP}.ods_activity_order where dt='$do_date' )act
...
from ${APP}.ods_user_info where dt='$do_date'
   ...
   select
uh.id,
uh.name, uh.birthday, uh.gender, uh.email, uh.user_level, uh.create_time,
uh.operate_time,
uh.start_date,
